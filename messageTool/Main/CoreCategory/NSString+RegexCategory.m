...
- (BOOL)isMobileNumberClassification
{
   ...
   */
    //    NSString * MOBILE =
    //    @"^1((3//d|5[0-35-9]|8[025-9])//d|70[059])\\d{7}$";//总况
   ...
   28         */
    ...
    if (([self isValidateByRegex:CM]) || ([self isValidateByRegex:CU]) ||
        ([self isValidateByRegex:CT]) || ([self isValidateByRegex:PHS])) {
        return YES;
    }
    else {
        return NO;
...
- (BOOL)isCarNumber
{
    ...
    NSString *carRegex =
        @"^[\u4e00-\u9fff]{1}[a-zA-Z]{1}[-][a-zA-Z_0-9]{4}[a-zA-Z_0-9_\u4e00-"
        @"\u9fff]$"; //其中\u4e00-\u9fa5表示unicode编码中汉字已编码部分，\u9fa5-\u9fff是保留部分，将来可能会添加
        ...
        firstCannotBeDigtal:(BOOL)firstCannotBeDigtal;
{
    ...
    NSString *regex =
        [NSString stringWithFormat:@"%@[%@A-Za-z0-9_]{%d,%d}", first, hanzi,
        ...
        firstCannotBeDigtal:(BOOL)firstCannotBeDigtal;
{
    NSString *hanzi = containChinese ? @"\u4e00-\u9fa5" : @"";
    NSString *first = firstCannotBeDigtal ? @"^[a-zA-Z_]" : @"";
    NSString *lengthRegex =
        [NSString stringWithFormat:@"(?=^.{%@,%@}$)", @(minLenth), @(maxLenth)];
    ...
    NSString *regex =
        [NSString stringWithFormat:@"%@%@%@%@", lengthRegex, digtalRegex,
                                   letterRegex, characterRegex];
    return [self isValidateByRegex:regex];
}

//判断是否包含 汉字、字母、数字 之外的特殊字符
    ...
    switch (length) {
        case 15:
            ...
            if (year % 4 == 0 || (year % 100 == 0 && year % 4 == 0)) {
                ...
                regularExpression = [[NSRegularExpression alloc]
                    initWithPattern:@"^[1-9][0-9]{5}[0-9]{2}((01|03|05|07|08|10|12)(0[1-"
        ...
        case 18:
            ...
            if (year % 4 == 0 || (year % 100 == 0 && year % 4 == 0)) {
                ...
                regularExpression = [[NSRegularExpression alloc]
                    initWithPattern:@"^[1-9][0-9]{5}19[0-9]{2}((01|03|05|07|08|10|12)(0["
...
/** 银行卡号有效性问题Luhn算法
 *  现行 16 位银联卡现行卡号开头 6 位是 622126～622925 之间的，7 到 15
 * 位是银行自定义的，
...
{
    NSString *lastNum =
        [[self substringFromIndex:(self.length - 1)] copy]; //取出最后一位
    NSString *forwardNum =
        [[self substringToIndex:(self.length - 1)] copy]; //前15或18位
    ...
    for (int i = (int)(forwardArr.count - 1); i > -1;
         i--) { //前15位或者前18位倒序存进数组
        [forwardDescArr addObject:forwardArr[i]];
    }

    NSMutableArray *arrOddNum =
        [[NSMutableArray alloc] initWithCapacity:0]; //奇数位*2的积 < 9
    NSMutableArray *arrOddNum2 =
        [[NSMutableArray alloc] initWithCapacity:0]; //奇数位*2的积 > 9
    NSMutableArray *arrEvenNum =
        [[NSMutableArray alloc] initWithCapacity:0]; //偶数位数组
    ...
    NSInteger luhmTotal =
        lastNumber + sumEvenNumTotal + sumOddNum2Total + sumOddNumTotal;
