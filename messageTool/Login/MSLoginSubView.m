...
@property (nonatomic, strong) UILabel *businessIdLabel;//用户id换成验证码
...
-(void)loginClick:(UIButton *)btn
{
    if (![self.userNameField.text isEqualToString:@""]) {
        if (![self.businessIdField.text isEqualToString:@""]) {
            if (![self.passWordField.text isEqualToString:@""]) {
                if (self.AgreementSelectBtn.selected) {
                    if (self.loginBlock) {
                        self.loginBlock(self.businessIdField.text, self.userNameField.text, self.passWordField.text);
                    }
                }else{
            ...
            [HUD showInfo:@"请输入企业ID" onView:LastWindow];
        }
    }else{
